...
pub struct TokenizerImpl<M, N, PT, PP, D> {
    ...
    // Which token plays which role ("eos_token" -> "</s>"), so a `tokenizer.json` can carry the
    // special-token metadata that used to require a separate `tokenizer_config.json`.
    role_to_token: Option<HashMap<String, String>>,
}
...
/// a valid chunk.
/// ```
/// use tk_encode::{Tokenizer, TokenizerBuilder, models::bpe::BPE, decoders::byte_fallback::ByteFallback, pre_tokenizers::byte_level::ByteLevel, normalizers::unicode::NFC};
/// use std::collections::HashMap;
...
///
/// ```
/// use tk_encode::{Tokenizer, TokenizerBuilder, models::bpe::BPE, pre_tokenizers::{byte_level::ByteLevel, metaspace::Metaspace}, normalizers::unicode::NFC};
/// use std::collections::HashMap;
/// use std::iter::FromIterator;
///
/// let vocab = HashMap::from_iter([
...
where
    ...
    PT: PreTokenizer,
{
    /// Normalize + pre-tokenize a raw `sequence` into the list of word strings a
    /// [`Trainer`](../../tk_train/trait.Trainer.html) consumes.
    ///
    /// This is the single piece of the training pipeline that needs the full
    /// (normalizer + added-vocabulary + pre-tokenizer) context, so it lives in
    /// `tk-encode` and is called by the training extension in `tk-train`.
